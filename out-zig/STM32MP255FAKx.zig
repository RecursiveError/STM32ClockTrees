...
const LVDSMult = struct {
  value: LVDSMultConf,
  pub fn get(comptime self: LVDSMult) comptime_int {
      return switch(self.value) {
.XBAR32Output => XBAR32Output_inst.get(),
 .HSEOSC => HSEOSC_inst.get(),
       };
   }
};
const LVDSoutput = struct{
  pub fn get(_:*const LVDSoutput) comptime_int{
      return LVDSMult_inst.get();
  }
};
...
const LVDSMult_inst = LVDSMult{ .value = conf.LVDSMult};
pub const LVDSoutput_inst = LVDSoutput{};
