...
const AHB2Prescaler = struct {
        value: AHB2PrescalerConf,

        pub fn get(self: *const AHB2Prescaler) comptime_int {
            if (!@hasDecl(this, "SysCLKOutput_inst")) {
...
const FCLK2CortexOutput = struct {
  pub fn get(_:*const FCLK2CortexOutput) comptime_int {
      const from_input = AHB2Prescaler_inst.get();
      if(from_input < 0){
          @compileError(std.fmt.comptimePrint("Underflow clock from AHB2Prescaler on FCLK2CortexOutput | recive {d} min 0\n", .{from_input}));
      }else if(from_input > 48000000){
          
          @compileError(std.fmt.comptimePrint("OverFlow clock from AHB2Prescaler on FCLK2CortexOutput | recive {d} max 48000000\n", .{from_input}));
      }
      return from_input;
  }
};
const AHB2Output = struct {
  pub fn get(_:*const AHB2Output) comptime_int {
      const from_input = AHB2Prescaler_inst.get();
      if(from_input < 0){
          @compileError(std.fmt.comptimePrint("Underflow clock from AHB2Prescaler on AHB2Output | recive {d} min 0\n", .{from_input}));
      }else if(from_input > 48000000){
          
          @compileError(std.fmt.comptimePrint("OverFlow clock from AHB2Prescaler on AHB2Output | recive {d} max 48000000\n", .{from_input}));
      }
      return from_input;
  }
};
const Cortex2Prescaler = struct {
        value: Cortex2PrescalerConf,

        pub fn get(self: *const Cortex2Prescaler) comptime_int {
            if (!@hasDecl(this, "AHB2Prescaler_inst")) {
              @compileError("No Input AHB2Prescaler for Cortex2Prescaler\n");
            }
            const from_input: comptime_float = @floatFromInt(this.AHB2Prescaler_inst.get());
            const div: comptime_float = self.value.get();
            return @intFromFloat((from_input / div));
        }
    };

const Cortex2SysOutput = struct {
  pub fn get(_:*const Cortex2SysOutput) comptime_int {
      const from_input = Cortex2Prescaler_inst.get();
      if(from_input < 0){
          @compileError(std.fmt.comptimePrint("Underflow clock from Cortex2Prescaler on Cortex2SysOutput | recive {d} min 0\n", .{from_input}));
      }else if(from_input > 48000000){
          
          @compileError(std.fmt.comptimePrint("OverFlow clock from Cortex2Prescaler on Cortex2SysOutput | recive {d} max 48000000\n", .{from_input}));
      }
      return from_input;
  }
};
...
const AHB2Prescaler_inst = AHB2Prescaler{ .value = conf.AHB2Prescaler};
pub const FCLK2CortexOutput_inst = FCLK2CortexOutput{};
pub const AHB2Output_inst = AHB2Output{};
const Cortex2Prescaler_inst = Cortex2Prescaler{ .value = conf.Cortex2Prescaler};
pub const Cortex2SysOutput_inst = Cortex2SysOutput{};
...
const PLLR_inst = PLLR{ .value = conf.PLLR};
 ...
 _ = FCLK2CortexOutput_inst.get();
 _ = AHB2Output_inst.get();
 _ = Cortex2SysOutput_inst.get();
