...
const SWPMIMult = struct {
  value: SWPMIMultConf,
  pub fn get(comptime self: SWPMIMult) comptime_int {
      return switch(self.value) {
.APB1Prescaler => APB1Prescaler_inst.get(),
 .HSIRC => HSIRC_inst.get(),
       };
...
const SWPMIoutput = struct{
  pub fn get(_:*const SWPMIoutput) comptime_int{
      return SWPMIMult_inst.get();
  }
};
...
const SDMMCoutput = struct {
  pub fn get(_:*const SDMMCoutput) comptime_int {
      const from_input = CK48Mult_inst.get();
      if(from_input < 0){
          @compileError(std.fmt.comptimePrint("Underflow clock from CK48Mult on SDMMCoutput | recive {d} min 0\n", .{from_input}));
      }else if(from_input > 50000000){
          
          @compileError(std.fmt.comptimePrint("OverFlow clock from CK48Mult on SDMMCoutput | recive {d} max 50000000\n", .{from_input}));
      }
      return from_input;
  }
};
...
const PLLSAI1N = struct {
        ...
        pub fn get(self: *const PLLSAI1N) comptime_int {
            if (!@hasDecl(this, "PLLM_inst")) {
              @compileError("No Input PLLM for PLLSAI1N\n");
            }
            const from_input: comptime_float = @floatFromInt(this.PLLM_inst.get());
            const multi = self.value.get();
            return @intFromFloat(from_input * multi);
...
const PLLSAI2N = struct {
        ...
        pub fn get(self: *const PLLSAI2N) comptime_int {
            if (!@hasDecl(this, "PLLM_inst")) {
              @compileError("No Input PLLM for PLLSAI2N\n");
            }
            const from_input: comptime_float = @floatFromInt(this.PLLM_inst.get());
            const multi = self.value.get();
            return @intFromFloat(from_input * multi);
...
const SWPMIMult_inst = SWPMIMult{ .value = conf.SWPMIMult};
pub const SWPMIoutput_inst = SWPMIoutput{};
