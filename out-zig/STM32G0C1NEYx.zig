...
const RTCWkupOutput = struct{
  pub fn get(_:*const RTCWkupOutput) comptime_int{
      return RTCClkSource_inst.get();
  }
};
...
const LPUART2Mult = struct {
  value: LPUART2MultConf,
  pub fn get(comptime self: LPUART2Mult) comptime_int {
      return switch(self.value) {
.APBPrescaler => APBPrescaler_inst.get(),
 .SysCLKOutput => SysCLKOutput_inst.get(),
 .HSIRC => HSIRC_inst.get(),
...
const LPUART2output = struct{
  pub fn get(_:*const LPUART2output) comptime_int{
      return LPUART2Mult_inst.get();
  }
};
const HSICECCDevisor = struct {

        pub fn get(_:*const HSICECCDevisor) comptime_int {
            if (!@hasDecl(this, "HSIRC_inst")) {
...
const CECMult = struct {
  ...
  pub fn get(comptime self: CECMult) comptime_int {
      return switch(self.value) {
.HSICECCDevisor => HSICECCDevisor_inst.get(),
 .LSEOSC => LSEOSC_inst.get(),
       };
   }
};
...
const TIM15Mult = struct {
  value: TIM15MultConf,
  pub fn get(comptime self: TIM15Mult) comptime_int {
      return switch(self.value) {
.TimPrescalerAPB => TimPrescalerAPB_inst.get(),
 .PLLQ => PLLQ_inst.get(),
       };
...
const TIM15output = struct{
  pub fn get(_:*const TIM15output) comptime_int{
      return TIM15Mult_inst.get();
  }
};
...
const RNGDIV = struct {
        ...
        pub fn get(self: *const RNGDIV) comptime_int {
            if (!@hasDecl(this, "CK48Mult_inst")) {
              @compileError("No Input CK48Mult for RNGDIV\n");
            }
            const from_input: comptime_float = @floatFromInt(this.CK48Mult_inst.get());
            const div: comptime_float = self.value.get();
            return @intFromFloat((from_input / div));
        }
    };

const RNGHSIDiv = struct {

        pub fn get(_:*const RNGHSIDiv) comptime_int {
            if (!@hasDecl(this, "HSIRC_inst")) {
...
.SysCLKOutput => SysCLKOutput_inst.get(),
 .PLLQ => PLLQ_inst.get(),
 .RNGHSIDiv => RNGHSIDiv_inst.get(),
       };
   }
};
const RNGoutput = struct {
  pub fn get(_:*const RNGoutput) comptime_int {
      const from_input = RNGoutput_inst.get();
      if(from_input < 0){
...
.LSEOSC => LSEOSC_inst.get(),
 ...
 .PLLP => PLLP_inst.get(),
 .PLLQ => PLLQ_inst.get(),
 .RTCClkSource => RTCClkSource_inst.get(),
 .RTCWkupOutput => RTCWkupOutput_inst.get(),
       };
   }
};
const MCODiv = struct {
...
pub const RTCWkupOutput_inst = RTCWkupOutput{};
...
const LPUART2Mult_inst = LPUART2Mult{ .value = conf.LPUART2Mult};
pub const LPUART2output_inst = LPUART2output{};
const HSICECCDevisor_inst = HSICECCDevisor{};
...
const TIM15Mult_inst = TIM15Mult{ .value = conf.TIM15Mult};
pub const TIM15output_inst = TIM15output{};
const ADCMult_inst = ADCMult{ .value = conf.ADCMult};
pub const ADCoutput_inst = ADCoutput{};
const RNGDIV_inst = RNGDIV{ .value = conf.RNGDIV};
const RNGHSIDiv_inst = RNGHSIDiv{};
...
const PLLR_inst = PLLR{ .value = conf.PLLR};
 ...
 _ = LPUART2output_inst.get();
 _ = CECoutput_inst.get();
 _ = TIM1output_inst.get();
 _ = TIM15output_inst.get();
