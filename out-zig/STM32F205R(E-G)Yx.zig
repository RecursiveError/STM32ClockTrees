...
pub const PLLNConf = enum(comptime_int) {
    ...
    pub fn get(num: PLLNConf) comptime_int {
        const val: comptime_int = @intFromEnum(num);
        if (val < 192) {
            @compileError("min value for PLLN is 192\n");
        } else if (val > 432) {
...
pub const PLLI2SNConf = enum(comptime_int) {
    ...
    pub fn get(num: PLLI2SNConf) comptime_int {
        const val: comptime_int = @intFromEnum(num);
        if (val < 192) {
            @compileError("min value for PLLI2SN is 192\n");
        } else if (val > 432) {
...
const SysCLKOutput = struct {
  pub fn get(_:*const SysCLKOutput) comptime_int {
      ...
      if(from_input < 0){
          ...
          @compileError(std.fmt.comptimePrint("OverFlow clock from SysClkSource on SysCLKOutput | recive {d} max 120000000\n", .{from_input}));
      }
      return from_input;
  }
};
...
const APB1Output = struct {
  pub fn get(_:*const APB1Output) comptime_int {
      ...
      }else if(from_input > 30000000){
          
          @compileError(std.fmt.comptimePrint("OverFlow clock from APB1Prescaler on APB1Output | recive {d} max 30000000\n", .{from_input}));
      }
      return from_input;
  }
};
...
const APB2Output = struct {
  pub fn get(_:*const APB2Output) comptime_int {
      ...
      if(from_input < 0){
          ...
          @compileError(std.fmt.comptimePrint("OverFlow clock from APB2Prescaler on APB2Output | recive {d} max 60000000\n", .{from_input}));
      }
      return from_input;
  }
};
