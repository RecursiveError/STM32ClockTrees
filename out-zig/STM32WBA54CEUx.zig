...
pub const LSI2RCConf = enum(comptime_int) {
    _,
    pub fn get(num: LSI2RCConf) comptime_int {
        const val: comptime_int = @intFromEnum(num);
        if (val < 31400) {
            @compileError("min value for LSI2RC is 31400\n");
        } else if (val > 32600) {
            @compileError("min value for LSI2RC is 32600\n");
        }
        return val;
    }
...
const LSI2RC = struct {
  value: LSI2RCConf,
  pub fn get(comptime self: LSI2RC) comptime_int { 
      return self.value.get(); 
  }
};
const LSIDIV = struct {
...
const LSIOut = struct {
  value: LSIOutConf,
  pub fn get(comptime self: LSIOut) comptime_int {
      return switch(self.value) {
.LSIDIV => LSIDIV_inst.get(),
 .LSI2RC => LSI2RC_inst.get(),
       };
   }
};
const LSEOSC = struct {
...
const LSI2RC_inst = LSI2RC{ .value = conf.LSI2RC};
