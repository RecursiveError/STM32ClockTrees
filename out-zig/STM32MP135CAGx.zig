...
pub fn Clock(comptime conf: Config) type {
    const ClockStruct = struct {
        ...
        const DCMIMultType = struct {
            value: DCMIMultConf,
            pub fn get(comptime self: DCMIMultType) comptime_int {
                return switch (self.value) {
                    .AXIOutput => AXIOutput.get(),
                    .DIVQ2 => DIVQ2.get(),
                    .DIVP4 => DIVP4.get(),
        ...
        const DCMIoutputType = struct {
            pub fn get(_: *const DCMIoutputType) comptime_int {
                const from_input = DCMIMult.get();
                if (from_input < 0) {
                    @compileError(std.fmt.comptimePrint("Underflow clock from DCMIMult on DCMIoutput | recive {d} min 0\n", .{from_input}));
                } else if (from_input > 266500000) {
                    @compileError(std.fmt.comptimePrint("OverFlow clock from DCMIMult on DCMIoutput | recive {d} max 266500000\n", .{from_input}));
                }
                return from_input;
            }
        };
        ...
        const SAESoutputType = struct {
            pub fn get(_: *const SAESoutputType) comptime_int {
                const from_input = SAESMult.get();
                if (from_input < 0) {
                    @compileError(std.fmt.comptimePrint("Underflow clock from SAESMult on SAESoutput | recive {d} min 0\n", .{from_input}));
                } else if (from_input > 360000000) {
                    @compileError(std.fmt.comptimePrint("OverFlow clock from SAESMult on SAESoutput | recive {d} max 360000000\n", .{from_input}));
                }
                return from_input;
            }
        };
        ...
        const DCMIMult = DCMIMultType{ .value = conf.DCMIMult };
        pub const DCMIoutput = DCMIoutputType{};
