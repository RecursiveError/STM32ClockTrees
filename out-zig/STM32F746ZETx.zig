...
const LCDTFTKOutput = struct {
  pub fn get(_:*const LCDTFTKOutput) comptime_int {
      const from_input = PLLSAIRDiv_inst.get();
      if(from_input < 0){
          @compileError(std.fmt.comptimePrint("Underflow clock from PLLSAIRDiv on LCDTFTKOutput | recive {d} min 0\n", .{from_input}));
      }else if(from_input > 45000000){
          
          @compileError(std.fmt.comptimePrint("OverFlow clock from PLLSAIRDiv on LCDTFTKOutput | recive {d} max 45000000\n", .{from_input}));
      }
      return from_input;
  }
};
const SPDIFoutput = struct {
  pub fn get(_:*const SPDIFoutput) comptime_int {
      const from_input = PLLI2SP_inst.get();
...
const HSIDivCEC = struct {

        pub fn get(_:*const HSIDivCEC) comptime_int {
...
pub const LCDTFTKOutput_inst = LCDTFTKOutput{};
...
const HSIDivCEC_inst = HSIDivCEC{};
