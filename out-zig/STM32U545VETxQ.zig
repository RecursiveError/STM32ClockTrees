...
const SHSIRC = struct {
  pub fn get(_:*const SHSIRC) comptime_int { 
      return 48000000; 
  }
};
const SHSIDiv = struct {

        pub fn get(_:*const SHSIDiv) comptime_int {
            if (!@hasDecl(this, "SHSIRC_inst")) {
              @compileError("No Input SHSIRC for SHSIDiv\n");
            }
            const from_input = this.SHSIRC_inst.get();
            const div = 2;
            return (from_input / div);
        }
    };

const SAESMult = struct {
  value: SAESMultConf,
  pub fn get(comptime self: SAESMult) comptime_int {
      return switch(self.value) {
.SHSIRC => SHSIRC_inst.get(),
 .SHSIDiv => SHSIDiv_inst.get(),
       };
   }
};
const SAESoutput = struct{
  pub fn get(_:*const SAESoutput) comptime_int{
      return SAESMult_inst.get();
  }
};
const HSEOSC = struct {
  value: HSEOSCConf,
...
const SHSIRC_inst = SHSIRC{};
const SHSIDiv_inst = SHSIDiv{};
const SAESMult_inst = SAESMult{ .value = conf.SAESMult};
