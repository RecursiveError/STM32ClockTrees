...
const DFSDM2AudioMult = struct {
  value: DFSDM2AudioMultConf,
  pub fn get(comptime self: DFSDM2AudioMult) comptime_int {
      return switch(self.value) {
.I2S1Mult => I2S1Mult_inst.get(),
 .I2S2Mult => I2S2Mult_inst.get(),
       };
...
const DFSDM2Audiooutput = struct{
  pub fn get(_:*const DFSDM2Audiooutput) comptime_int{
      return DFSDM2AudioMult_inst.get();
  }
};
const SAI1AMult = struct {
  value: SAI1AMultConf,
  pub fn get(comptime self: SAI1AMult) comptime_int {
      return switch(self.value) {
.PLLSource => PLLSource_inst.get(),
 .SAI1APrescaler => SAI1APrescaler_inst.get(),
 .SAI1BPrescaler => SAI1BPrescaler_inst.get(),
 .I2S_CKIN => I2S_CKIN_inst.get(),
       };
   }
};
const SAI1Aoutput = struct{
  pub fn get(_:*const SAI1Aoutput) comptime_int{
      return SAI1AMult_inst.get();
  }
};
const SAI1APrescaler = struct {
        value: SAI1APrescalerConf,

        pub fn get(self: *const SAI1APrescaler) comptime_int {
            if (!@hasDecl(this, "PLLR_inst")) {
              @compileError("No Input PLLR for SAI1APrescaler\n");
            }
            const from_input: comptime_float = @floatFromInt(this.PLLR_inst.get());
            const div = self.value.get();
            return @intFromFloat(from_input / div);
        }
    };

const SAI1BMult = struct {
  value: SAI1BMultConf,
  pub fn get(comptime self: SAI1BMult) comptime_int {
      return switch(self.value) {
.PLLSource => PLLSource_inst.get(),
 .SAI1APrescaler => SAI1APrescaler_inst.get(),
 .SAI1BPrescaler => SAI1BPrescaler_inst.get(),
...
const SAI1Boutput = struct{
  pub fn get(_:*const SAI1Boutput) comptime_int{
      return SAI1BMult_inst.get();
  }
};
const SAI1BPrescaler = struct {
        value: SAI1BPrescalerConf,

        pub fn get(self: *const SAI1BPrescaler) comptime_int {
            if (!@hasDecl(this, "PLLI2SR_inst")) {
              @compileError("No Input PLLI2SR for SAI1BPrescaler\n");
            }
            const from_input: comptime_float = @floatFromInt(this.PLLI2SR_inst.get());
            const div = self.value.get();
            return @intFromFloat(from_input / div);
        }
    };

const LPTimerMult = struct {
  value: LPTimerMultConf,
  pub fn get(comptime self: LPTimerMult) comptime_int {
      return switch(self.value) {
.APB1Prescaler => APB1Prescaler_inst.get(),
 .LSIRC => LSIRC_inst.get(),
 .HSIRC => HSIRC_inst.get(),
...
const LPTimeroutput = struct{
  pub fn get(_:*const LPTimeroutput) comptime_int{
      return LPTimerMult_inst.get();
  }
};
const PLLN = struct {
        value: PLLNConf,
...
const DFSDM2AudioMult_inst = DFSDM2AudioMult{ .value = conf.DFSDM2AudioMult};
pub const DFSDM2Audiooutput_inst = DFSDM2Audiooutput{};
const SAI1AMult_inst = SAI1AMult{ .value = conf.SAI1AMult};
pub const SAI1Aoutput_inst = SAI1Aoutput{};
const SAI1APrescaler_inst = SAI1APrescaler{ .value = conf.SAI1APrescaler};
const SAI1BMult_inst = SAI1BMult{ .value = conf.SAI1BMult};
pub const SAI1Boutput_inst = SAI1Boutput{};
const SAI1BPrescaler_inst = SAI1BPrescaler{ .value = conf.SAI1BPrescaler};
const LPTimerMult_inst = LPTimerMult{ .value = conf.LPTimerMult};
pub const LPTimeroutput_inst = LPTimeroutput{};
...
const PLLI2SR_inst = PLLI2SR{ .value = conf.PLLI2SR};
 ...
 _ = DFSDM2Audiooutput_inst.get();
 _ = SAI1Aoutput_inst.get();
 _ = SAI1Boutput_inst.get();
 _ = LPTimeroutput_inst.get();
