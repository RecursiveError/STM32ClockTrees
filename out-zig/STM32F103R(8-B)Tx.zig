...
pub const Config = struct {
    ...
    PLLSource: PLLSourceConf = .HSEDivPLL,
...
pub fn Clock(comptime conf: Config) type {
    const ClockStruct = struct {
        ...
        const MCOMultDivisor = struct {
            pub fn get() comptime_int {
                if (!@hasDecl(@TypeOf(this), "PLLMUL_inst")) {
        ...
        const APB1Output = struct {
            pub fn get() comptime_int {
                ...
                if (from_input < 10000000) {
                    @compileError(std.fmt.comptimePrint("Underflow clock from APB1Prescaler on APB1Output | recive {d} min 10000000\n", .{from_input}));
                } else if (from_input > 36000000) {
        ...
        const TimPrescalerAPB1 = struct {
            pub fn get() comptime_int {
                if (!@hasDecl(@TypeOf(this), "APB1Prescaler_inst")) {
        ...
        const TimPrescalerAPB2 = struct {
            pub fn get() comptime_int {
                if (!@hasDecl(@TypeOf(this), "APB2Prescaler_inst")) {
        ...
        const VCO2output = struct {
            pub fn get() comptime_int {
                const from_input = PLLSource_inst.get();
                if (from_input < 1000000) {
                    @compileError(std.fmt.comptimePrint("Underflow clock from PLLSource on VCO2output | recive {d} min 1000000\n", .{from_input}));
                } else if (from_input > 25000000) {
                    @compileError(std.fmt.comptimePrint("OverFlow clock from PLLSource on VCO2output | recive {d} max 25000000\n", .{from_input}));
                }
                return from_input;
            }
        };
        const PLLMUL = struct {
            ...
            pub fn get(self: *const PLLMUL) comptime_int {
                if (!@hasDecl(@TypeOf(this), "VCO2output_inst")) {
        ...
        const LSEOSC_inst = LSEOSC{};
        const HSEOSC_inst = HSEOSC{ .value = conf.HSEOSCConf };
        const HSEDivPLL_inst = HSEDivPLL{ .value = conf.HSEDivPLLConf };
        ...
        pub const RTCOutput_inst = RTCOutput{ .value = conf.RTCOutputConf };
        pub const IWDGOutput_inst = IWDGOutput{};
        const MCOMultDivisor_inst = MCOMultDivisor{};
        const MCOMult_inst = MCOMult{ .value = conf.MCOMultConf };
        pub const MCOoutput_inst = MCOoutput{ .value = conf.MCOoutputConf };
        const AHBPrescaler_inst = AHBPrescaler{ .value = conf.AHBPrescalerConf };
        ...
        const TimSysPresc_inst = TimSysPresc{ .value = conf.TimSysPrescConf };
        pub const TimSysOutput_inst = TimSysOutput{};
        const APB1Prescaler_inst = APB1Prescaler{ .value = conf.APB1PrescalerConf };
        ...
        const APB2Prescaler_inst = APB2Prescaler{ .value = conf.APB2PrescalerConf };
        ...
        const ADCprescaler_inst = ADCprescaler{ .value = conf.ADCprescalerConf };
        pub const ADCoutput_inst = ADCoutput{ .value = conf.ADCoutputConf };
        const USBPrescaler_inst = USBPrescaler{ .value = conf.USBPrescalerConf };
        pub const USBoutput_inst = USBoutput{ .value = conf.USBoutputConf };
        const PLLSource_inst = PLLSource{ .value = conf.PLLSourceConf };
        pub const VCO2output_inst = VCO2output{ .value = conf.VCO2outputConf };
        const PLLMUL_inst = PLLMUL{ .value = conf.PLLMULConf };
