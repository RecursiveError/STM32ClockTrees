...
const PLLSAIM = struct {
        value: PLLSAIMConf,

        pub fn get(self: *const PLLSAIM) comptime_int {
            if (!@hasDecl(this, "PLLSource_inst")) {
...
const PLLSAIN = struct {
        ...
        pub fn get(self: *const PLLSAIN) comptime_int {
            if (!@hasDecl(this, "PLLSAIM_inst")) {
              @compileError("No Input PLLSAIM for PLLSAIN\n");
            }
            const from_input: comptime_float = @floatFromInt(this.PLLSAIM_inst.get());
            const multi = self.value.get();
            return @intFromFloat(from_input * multi);
        }
    };
...
const PLLSAIM_inst = PLLSAIM{ .value = conf.PLLSAIM};
