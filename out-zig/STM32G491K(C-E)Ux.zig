...
const IWDGOutput = struct {
  pub fn get(_:*const IWDGOutput) comptime_int {
      const from_input = LSIRC_inst.get();
      if(from_input < 0){
...
const ADC345Mult = struct {
  value: ADC345MultConf,
  pub fn get(comptime self: ADC345Mult) comptime_int {
      return switch(self.value) {
.SysCLKOutput => SysCLKOutput_inst.get(),
 .PLLP => PLLP_inst.get(),
       };
...
const ADC345output = struct {
  pub fn get(_:*const ADC345output) comptime_int {
      const from_input = ADC345Mult_inst.get();
      if(from_input < 0){
...
const HRTIM1CLKoutput = struct{
  pub fn get(_:*const HRTIM1CLKoutput) comptime_int{
      return TimPrescOut2_inst.get();
  }
};
...
const ADC345Mult_inst = ADC345Mult{ .value = conf.ADC345Mult};
pub const ADC345output_inst = ADC345output{};
...
pub const HRTIM1CLKoutput_inst = HRTIM1CLKoutput{};
