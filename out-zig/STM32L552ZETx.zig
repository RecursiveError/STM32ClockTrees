...
const PLLSAI1Source = struct {
  value: PLLSAI1SourceConf,
  pub fn get(comptime self: PLLSAI1Source) comptime_int {
      return switch(self.value) {
.MSIRC => MSIRC_inst.get(),
 .HSIRC => HSIRC_inst.get(),
 .HSEOSC => HSEOSC_inst.get(),
...
const PLLSAI2Source = struct {
  value: PLLSAI2SourceConf,
  pub fn get(comptime self: PLLSAI2Source) comptime_int {
      return switch(self.value) {
.MSIRC => MSIRC_inst.get(),
 .HSIRC => HSIRC_inst.get(),
 .HSEOSC => HSEOSC_inst.get(),
...
const PLLSAI1M = struct {
        value: PLLSAI1MConf,

        pub fn get(self: *const PLLSAI1M) comptime_int {
            if (!@hasDecl(this, "PLLSAI1Source_inst")) {
              @compileError("No Input PLLSAI1Source for PLLSAI1M\n");
            }
            const from_input: comptime_float = @floatFromInt(this.PLLSAI1Source_inst.get());
            const div = self.value.get();
            return @intFromFloat(from_input / div);
        }
    };

const PLLSAI2M = struct {
        value: PLLSAI2MConf,

        pub fn get(self: *const PLLSAI2M) comptime_int {
            if (!@hasDecl(this, "PLLSAI2Source_inst")) {
              @compileError("No Input PLLSAI2Source for PLLSAI2M\n");
            }
            const from_input: comptime_float = @floatFromInt(this.PLLSAI2Source_inst.get());
            const div = self.value.get();
            return @intFromFloat(from_input / div);
        }
    };
...
const PLLSAI1N = struct {
        ...
        pub fn get(self: *const PLLSAI1N) comptime_int {
            if (!@hasDecl(this, "PLLSAI1M_inst")) {
              @compileError("No Input PLLSAI1M for PLLSAI1N\n");
            }
            const from_input: comptime_float = @floatFromInt(this.PLLSAI1M_inst.get());
            const multi = self.value.get();
            return @intFromFloat(from_input * multi);
        }
    };
...
const PLLSAI2N = struct {
        ...
        pub fn get(self: *const PLLSAI2N) comptime_int {
            if (!@hasDecl(this, "PLLSAI2M_inst")) {
              @compileError("No Input PLLSAI2M for PLLSAI2N\n");
            }
            const from_input: comptime_float = @floatFromInt(this.PLLSAI2M_inst.get());
            const multi = self.value.get();
            return @intFromFloat(from_input * multi);
        }
    };
...
const PLLSAI1Source_inst = PLLSAI1Source{ .value = conf.PLLSAI1Source};
const PLLSAI2Source_inst = PLLSAI2Source{ .value = conf.PLLSAI2Source};
const PLLM_inst = PLLM{ .value = conf.PLLM};
const PLLSAI1M_inst = PLLSAI1M{ .value = conf.PLLSAI1M};
const PLLSAI2M_inst = PLLSAI2M{ .value = conf.PLLSAI2M};
